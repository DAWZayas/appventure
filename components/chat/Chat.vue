<template>
  <div d-flex> 
    <v-list two-linev class="mx-2 pb-0"  v-if="this.type === 'Venture'">
      <template>
    ...
    <v-list two-linev class="mx-2 pb-0 pt- px-2">
      <template  v-for="(message, index, key) in this.type === 'Venture' ? this.messages : users[id].messages"  :message="message">
...
<script>
  ...
  export default {
    props: ['id'],
    computed: {
      ...mapGetters({userData: 'getUserData', user: 'getUser', users: 'getUsers', type: 'getUserType', messages: 'getUserMessages'})
    },
    methods: {
      ...mapActions(['addMessage']),
      pushMessage () {
        this.newMessage === '' ? null : this.addMessage({newMessage: this.newMessage, issuing: this.user.uid, uidUser: this.type === 'Venture' ? this.user.uid : this.id})
        this.newMessage = ''
      }
    },
    data () {
